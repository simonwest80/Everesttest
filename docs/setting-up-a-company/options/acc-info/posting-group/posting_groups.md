...
{:.note}
Posting groups are not mandatory. However, while migrating or importing  data to 3.0 some Posting Groups do get created based on the data in the  legacy company. These Posting Groups may have some or all the accounts  defined. This data for the Posting Groups is from the accounting defaults  of the legacy data. Irrespective of whether all accounts are defined or  not, the company accounting defaults display these Posting Groups. This  is an exception to the rule that Posting Groups need to have all accounts  defined for them to enable them to be selected as a company default.


![]({{site.sc_baseurl}}/img/lens.gif)[Posting  Groups Browser]({{site.sc_baseurl}}/options/acc-info/posting-group/posting_group_browser_accounting_setup.html)


Similarly while creating a new transaction, it is not mandatory for  you to create Posting Group.


However, during any sales/purchase transaction or while creating a manual  journal etc. if the account that needs to be used in the transaction has  not been defined in any Posting Group, you will get an error message.  This indicates that you need to define the account at company level before  proceeding with that particular transaction.
