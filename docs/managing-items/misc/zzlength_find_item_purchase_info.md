...
Select the parameter based on which to perform the item search (by length).


If the parameter selected requires a range to be specified, both the  fields beside the **Length** field  are enabled. Enter the starting value in the first field and the ending  value in the second field.
